...
const Afternoon = ({id,title: initialTitle,notes: initialNotes,removePlanA,donePlanA,done, onEdit}) => {
...
return ( 
    <div className='col-md-4 text-wrap'>
        ...
        <button type='button' className='ml-auto p-2 btn btn-outline-secondary' onClick={() => removePlanA(id)}>delete</button>
        <button type='button' className='btn btn-outline-info btn-block' onClick={() => donePlanA(id)}>done</button>
        </div>
        </section>
)}
</div>
