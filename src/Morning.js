...
const Morning = ({id,title: initialTitle,notes: initialNotes,removePlanM,donePlanM,done,onEdit}) => {
...
return ( 
    <div className='col-md-4 text-wrap'>
        ...
        <button type='button' className='ml-auto p-2 btn btn-outline-secondary' onClick={() => removePlanM(id)}>delete</button>
        <button type='button' className='btn btn-outline-info btn-block' onClick={() => donePlanM(id)}>done</button>
        </div>
        </section>
)}
</div>
